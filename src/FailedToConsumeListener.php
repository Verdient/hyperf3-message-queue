...
use Verdient\MessageQueue\Event\FailedToConsume;
 ...
 * @author Verdient。
 */
class FailedToConsumeListener implements ListenerInterface
{
    /**
     * 缓存的日志记录器
     *
     ...
     * 获取记录器
     *
     * @param FailedToConsume $event 事件
     *
     * @author Verdient。
     */
    protected function getLogger(FailedToConsume $event): LoggerInterface
    {
    ...
    public function process(object $event): void
    {
        ...
        $this->getLogger($event)->info(sprintf('消息 %s 消费失败，耗时 %.4f 秒。', $event->message::class, $cost));

        if ($event->message instanceof AbstractMessage) {
            $logger2 = $event->message->logger();
            $logger2->error($event->throwable);
            $logger2->info(sprintf('消费失败，耗时 %.4f 秒。', $cost));
        }
    }
}
